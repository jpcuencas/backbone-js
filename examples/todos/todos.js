// Una aplicación Backbone ejemplo aportado por
// [Jérôme Gravel-Niquet] (http://jgn.me/). Esta demo utiliza un sencillo
// [Adaptador LocalStorage] (backbone-localstorage.html)
// A persistir modelos Backbone en su navegador.

// Cargar la aplicación una vez que el DOM esté listo, utilizando`jQuery.ready`:
$(function(){
  ...
  var Todo = Backbone.Model.extend({
    ...
    // atributos por defecto todo item.
    defaults: function() {
      return {
        title: "empty todo...",
    ...
    // Asegúrese de que cada TODO creada tiene `title`.
    initialize: function() {
      if (!this.get("title")) {
...
// La colección de Todos con el respaldo de * localStorage * en lugar de un control remoto
   // Servidor.
  var TodoList = Backbone.Collection.extend({
    ...
    // Filtrar la lista de todos los elementos de tareas pendientes que están terminados.
    done: function() {
      return this.filter(function(todo){ return todo.get('done'); });
    },

    // Filtrar la lista de tareas pendientes únicos elementos que todavía no han terminado.
    remaining: function() {
  ...
  var PendingItems = Backbone.View.extend({
    ...
    var DoneItems = Backbone.View.extend({
		...
		render: function() {
			...
			var pending = Todos.where({done: true});
			var self = this;
			if(pending.length > "0"){
				_.each(Todos.models, function(model){
					if(model.get("done")){
